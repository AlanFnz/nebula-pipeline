...
def particle_field(p, time, seed):
    ...
    points = target * cohesion[:, None] + loose * (1 - cohesion[:, None])
    motion_phase = attributes[:, 8:11] * math.tau
    flow = np.sin(target[:, [1, 2, 0]] * 3.4 + motion_phase + time * p["flow"])
    flow += .4 * np.sin(target[:, [2, 0, 1]] * 6.2 - motion_phase + time * p["flow"] * .63)
